...
class Coupling(nn.Module):
    ...
    Args:
        ...
        norm (str): Normalization to be used ('batchnorm' or 'actnorm').
            in NN.
    """

    def __init__(self, in_channels, mid_channels, norm="batchnorm"):
        super(Coupling, self).__init__()
        assert (
            norm == "batchnorm" or norm == "actnorm"
        ), "Only batchnorm or actnorm is supported in Coupling layer!"

        self.nn = NN(
            in_channels,
            mid_channels,
            2 * in_channels,
            use_act_norm=(norm == "actnorm"),
        )
...
class NN(nn.Module):
    ...
    def __init__(
        ...
        nn.init.normal_(self.in_conv.weight, 0.0, 0.05)
        self.in_norm = norm_fn(mid_channels)

        self.mid_conv = nn.Conv2d(
            mid_channels, mid_channels, kernel_size=1, padding=0, bias=False
        )
        nn.init.normal_(self.mid_conv.weight, 0.0, 0.05)
